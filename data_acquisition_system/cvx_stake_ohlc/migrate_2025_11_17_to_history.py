#!/usr/bin/env python3
# =====================================
# 2025年11月17日のCvxStakeMetricsデータをCvxStakeHistoryに移行
# =====================================
...
class Migrate20251117:
    def __init__(self):
        """初期化"""
        self.dynamodb = boto3.resource('dynamodb', region_name='ap-northeast-1')
        self.JST = timezone(timedelta(hours=9))
        self.target_date = datetime(2025, 11, 17, tzinfo=self.JST)
    ...
    def setup_tables(self):
        """テーブルに接続"""
        try:
            self.metrics_table = self.dynamodb.Table('CvxStakeMetrics')
            self.metrics_table.load()
            print("✅ CvxStakeMetricsテーブルに接続しました")
            
            self.history_table = self.dynamodb.Table('CvxStakeHistory')
            self.history_table.load()
            print("✅ CvxStakeHistoryテーブルに接続しました")
    ...
    def migrate(self):
        ...
        try:
            ...
            print(f"✅ {saved_count}件のデータをCvxStakeHistoryに保存しました")
            return True
            ...
            if self.slack_notifier:
                self.slack_notifier.notify_error(
                    message=error_msg,
                    system_name="2025-11-17 Migration",
                    error=e
                )
            return False
