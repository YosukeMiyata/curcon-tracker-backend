#!/usr/bin/env python3
# =====================================
# CvxStakeMetricsからCvxStakeOHLCDailyへのデータ移行スクリプト
# CvxStakeMetricsテーブルのデータ（2025年11月16日まで）を
# CvxStakeOHLCDailyテーブルのOHLC形式に集約して保存
# vaprとtvlの2つのtypeでそれぞれ集約
# =====================================
...
class CvxStakeToOHLCMigrator:
    def __init__(self):
        """CvxStakeMetricsからCvxStakeOHLCDailyへの移行システムの初期化"""
        self.dynamodb = boto3.resource('dynamodb', region_name='ap-northeast-1')
        self.cvx_stake_table = None
        ...
        # 終了日（2025年11月16日）
        self.end_date = datetime(2025, 11, 16, 23, 59, 59, tzinfo=self.JST)
    ...
    def setup_logging(self):
        """ログ設定"""
        log_file = Path(__file__).parent / 'migrate_cvx_stake_to_ohlc.log'
        logging.basicConfig(
            level=logging.INFO,
    ...
    def setup_tables(self):
        """DynamoDBテーブルに接続"""
        try:
            self.cvx_stake_table = self.dynamodb.Table('CvxStakeMetrics')
            self.cvx_stake_table.load()
            self.logger.info("✅ CvxStakeMetricsテーブルに接続しました")
            
            self.ohlc_table = self.dynamodb.Table('CvxStakeOHLCDaily')
            self.ohlc_table.load()
            self.logger.info("✅ CvxStakeOHLCDailyテーブルに接続しました")
            ...
            if self.slack_notifier:
                self.slack_notifier.notify_error(
                    message=error_msg,
                    system_name="CvxStake to OHLC Migrator",
                    error=e
                )
            raise e
    
    def get_cvx_stake_data(self):
        """CvxStakeMetricsテーブルからCVXのデータを取得（2025年11月16日まで）"""
        try:
            self.logger.info("📊 CvxStakeMetricsテーブルからデータ取得中...")
            ...
            while True:
                ...
                response = self.cvx_stake_table.query(**query_params)
    ...
    def check_existing_data(self, type_name: str, date_key: str) -> bool:
        """CvxStakeOHLCDailyテーブルに既存データがあるかチェック"""
        try:
    ...
    def save_ohlc_data(self, ohlc_data: Dict[str, Dict], type_name: str):
        """OHLCデータをCvxStakeOHLCDailyテーブルに保存"""
        if not ohlc_data:
            self.logger.warning(f"⚠️ 保存する{type_name}用のOHLCデータがありません")
            return False
        
        try:
            self.logger.info(f"💾 CvxStakeOHLCDailyテーブルに{type_name}データ保存中...")
    ...
    def run_migration(self):
        """移行処理を実行"""
        self.logger.info("🚀 CvxStakeMetricsからCvxStakeOHLCDailyへの移行開始")
        self.logger.info("=" * 60)
        
        try:
            # 1. CvxStakeMetricsデータを取得
            items = self.get_cvx_stake_data()
            
            if not items:
                self.logger.warning("⚠️ 取得したデータが1件もありません。処理を終了します。")
                return False
            
            # 2. vaprとtvlの2つのtypeでOHLCデータを集約
            self.logger.info("📊 OHLCデータ集約中...")
            
            vapr_ohlc_data = self.aggregate_ohlc_data(items, 'vapr')
            tvl_ohlc_data = self.aggregate_ohlc_data(items, 'tvl')
            
            if not vapr_ohlc_data and not tvl_ohlc_data:
            ...
            # 3. vaprデータを保存
            if vapr_ohlc_data:
                if not self.save_ohlc_data(vapr_ohlc_data, 'vapr'):
                    error_msg = "❌ vapr OHLCデータ保存に失敗しました。"
                    self.logger.error(error_msg)
                    if self.slack_notifier:
                        self.slack_notifier.notify_error(
                            message=error_msg,
                ...
                self.logger.warning("⚠️ vapr用のOHLCデータがありません")
            ...
            if self.slack_notifier:
                self.slack_notifier.notify_success(
                    message="CvxStakeMetricsからCvxStakeOHLCDailyへの移行が完了しました",
                    system_name="CvxStake to OHLC Migrator"
                )
...
def main():
    """メイン処理"""
    migrator = CvxStakeToOHLCMigrator()
